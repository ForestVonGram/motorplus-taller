...
export default function ClientesDetail() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setLoading(true);
      try {
        const res = await fetch(`${API_BASE}/api/clientes/${encodeURIComponent(id || '')}`, {
        ...
        if (alive) setError(e?.message || 'Error cargando cliente');
      } finally {
  ...
  const onDelete = async () => {
    if (!id) return;
    const ok = window.confirm(`¿Eliminar cliente ${id}? Esta acción no se puede deshacer.`);
    if (!ok) return;
    setSaving(true);
    setError(null);
    try {
    ...
    return (
      <div className="vehiculos-page">
        ...
        <div style={{ marginTop: 16 }}>
          <button className="btn-secondary" onClick={() => navigate('/clientes')}>Volver al listado</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="vehiculos-page">
      <div className="page-header">
        <h1>Cliente {id}</h1>
        <div>
          <button className="btn-secondary" onClick={() => navigate('/clientes')}>Volver</button>
        </div>
      </div>
      ...
      <form className="form-card" onSubmit={onSubmit}>
        <div className="form-row">
          ...
          <input name="idCliente" value={form.idCliente} disabled />
        </div>
        <div className="form-row">
          <label>Nombre</label>
        ...
        <div className="form-actions">
          ...
          <div style={{ flex: 1 }} />
          <button type="button" className="btn-secondary" onClick={() => navigate('/clientes')} disabled={saving}>
            Cancelar
          </button>
